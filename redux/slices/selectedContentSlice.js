...
export const fetchArticlesByCategory = createAsyncThunk(
    ...
    async ({ categoryUrl, page }, thunkAPI) => {
        try {
            const response = await axios.get(`${API_URL}/articles/mob/${categoryUrl}/${page}`);
            const data = response.data;
...
export const { appendContentData,appendNajnovijeData,setScrollPosition,setCurrentPage, setHasMore,setMainArticles,setSelectedCategory, setAllCategories,setLoading ,setNajnovijeData, setContentData, setHighlightData } = selectedContentSlice.actions;
