...
const CategoryContent = ({loadMore, isPageLoading}) => {
    ...
    const { selectedCategory, currentPage } = useSelector((state) => state.selectedContent);


    const loadMoreContent = () => {
            const nextPage = currentPage + 1;
            dispatch(fetchArticlesByCategory({ categoryUrl: selectedCategory, page: nextPage }));
    };
    ...
    return (
        <View style={styles.container }>
            ...
            <View>
                <Newsfeed onEndReached={loadMoreContent} isPageLoading={isPageLoading} />
            </View>
        </View>
    );
};
