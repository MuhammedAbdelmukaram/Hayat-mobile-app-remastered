...
const CategoryHighlightNews = () => {
  const mainArticles = useSelector(
    (state) => state.selectedContent.mainArticles
  );
    ...
    return selectedArticles
      .map((article, index) => {
        ...
        return PriorityComponent
          ? React.createElement(PriorityComponent, {
              key: article._id,
  ...
  const applyRenderingLogic = (articles, categoryIndex) => {
    ...
    switch (algoIndex) {
      case 0:
        return renderArticlesAlgo1(articles);
      case 1:
        return renderArticlesAlgo2(articles);
      case 2:
        return renderArticlesAlgo3(articles);
      default:
        return null;
    }
  };
